...
# ──────────────────────────────────────────────────────────────
# What-If injection helper (additive — does NOT alter the
# existing burst-injection logic in generate_schedule)
# ──────────────────────────────────────────────────────────────

_inject_counter = 0


def inject_vessel(vessels: List[Vessel], vessel: Vessel) -> List[Vessel]:
    """
    Return a NEW list containing all existing vessels plus *vessel*.

    This is a pure function: it does not modify the original list so
    the caller can keep a clean pre-injection copy for diff computation.

    Parameters
    ----------
    vessels : List[Vessel]
        The existing schedule produced by generate_schedule().
    vessel : Vessel
        A fully constructed Vessel instance to append.

    Returns
    -------
    List[Vessel]
        A new list = vessels + [vessel].
    """
    return list(vessels) + [vessel]


def make_injected_vessel(
    vessel_class: VesselClass,
    eta_hour: int,
    priority: Priority,
    cargo_volume_teu: int,
    base_ts: float,
) -> Vessel:
    """
    Construct a single Vessel for what-if injection.

    vessel_id and name are auto-generated; the caller does not supply them.
    base_ts is the simulation start timestamp so ETA is consistent with
    the rest of the schedule.
    """
    global _inject_counter
    _inject_counter += 1
    vessel_id = f"V-INJECT-{_inject_counter:03d}"
    return Vessel(
        vessel_id=vessel_id,
        name=f"Injected Vessel {_inject_counter:03d}",
        vessel_class=vessel_class,
        cargo_volume_teu=cargo_volume_teu,
        eta=base_ts + eta_hour * SECONDS_PER_HOUR,
        priority=priority,
    )
